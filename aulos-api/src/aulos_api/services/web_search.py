...
def search_duckduckgo(*, query: str, limit: int = 5, timeout: float = 12.0) -> list[dict[str, Any]]:
...
def search_brave(*, query: str, api_key: str, limit: int = 5, timeout: float = 12.0) -> list[dict[str, Any]]:
    ...
    if not q or not key:
        return []
    try:
        with httpx.Client(timeout=timeout, headers={"User-Agent": _UA, "X-Subscription-Token": key}) as client:
            r = client.get(
...
def fetch_jina_reader(*, url: str, limit: int = 1200, timeout: float = 20.0) -> dict[str, Any] | None:
    ...
    try:
        with httpx.Client(timeout=timeout, headers={"User-Agent": _UA, "Accept": "text/plain"}, follow_redirects=True) as client:
    ...
    *,
    ...
    for row in sources:
        ...
        if str(row.get("provider") or "") == "agent-reach-jina":
            continue
        url = str(row.get("url") or "")
        deepened = fetch_jina_reader(url=url, timeout=timeout)
        if not deepened:
            continue
...
def gather_web_sources(
    *,
    ...
    wiki_limit = max(1, min(3, int(wikipedia_limit or 3)))
    for variant in variants:
        add_all(search_wikipedia(query=variant, limit=wiki_limit, timeout=search_timeout))
        if len(out) >= max_sources:
            break
    if len(out) < 2:
        add_all(search_duckduckgo(query=f"{variants[-1]} classical music", timeout=search_timeout))
    if brave_api_key:
        add_all(search_brave(query=query, api_key=brave_api_key, timeout=search_timeout))
    if use_reach and out:
        deepen_with_agent_reach(out, max_pages=2, timeout=jina_timeout)
    if not out:
